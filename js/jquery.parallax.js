...
(function(jQuery, undefined) {
	...
	function followPointer(targetPointer, prevPointer, threshold, decay, parallax, targetFn, updateFn) {
		var pointer, x;
		...
		if ((!parallax[0] || Math.abs(targetPointer[0] - prevPointer[0]) < threshold[0]) &&
			...
			// Pointer has hit the target, so do nothing (just idle).
	...
	jQuery.fn.parallax = function(o){
		...
		return this.each(function(i) {
			var node      = this,
				...
				targetFn  = null, // Function which should be called once target is reached, if applicable.
				events = {
					...
					'mouseleave.parallax': function mouseleave(e) {
						// Now that user has moved mouse out of view port, completely stop timer, but only do so when
			...
			function frame() {
				...
				followPointer(port.pointer, pointer, port.threshold, decay, parallax, targetFn, updateCss, elem);
			}
